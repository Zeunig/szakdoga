                ...
                <tbody className="">
                    {reports.map((report) => (
                        <tr key={report.car.id} className="odd:bg-blue-100 even:bg-blue-200 ">
                            ...
                            <td className="">
                                <button onClick={() => { axios.delete(`/api/cars/${report.car.id}`);axios.post("/api/report/solve", {"reason_id": report.id}); reports.map((r) => { report.car.id !== r.car.id }) }} className="mx-5 duration-300  hover:scale-125"> <X className="text-red-600"/> </button>

                                <button onClick={() => { axios.post(`/api/admin/ban`, { "user_id": report.car.seller_id }); axios.post("/api/report/solve", {"reason_id": report.id}); reports.map((r) => { report.car.id !== r.car.id }) }} className="mx-5 duration-300 hover:scale-125"> <Hammer /> </button>
                            </td>
                        </tr>
                    ))}
                </tbody>
